...
class _ShowProductSellerState extends State<ShowProductSeller> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: ShowButton(
        label: 'Add New Plant',
        pressFunc: () {
          Navigator.pushNamed(context, MyConstant.routhAddProduct)
              .then((value) {
            readPlant();
          });
        },
      ...
      body: load
          ? const ShowProgress()
          : havePlant!
              ? LayoutBuilder(builder: (context, contants) {
                  return listPlant(contants);
                })
              : Center(
                  child: ShowTitle(
  ...
  Widget listPlant(BoxConstraints contants) => ListView.builder(
        itemCount: plantModels.length,
        itemBuilder: (context, index) => Card(
          child: Row(
            children: [
              SizedBox(
                width: contants.maxWidth * 0.5 - 4,
                height: contants.maxWidth * 0.3,
                child: Padding(
